...
var Widget = React.createClass({
	  ...
	  this.DetailWidget = WidgetList[this.props.widget.type];
	    if (!this.DetailWidget) {
			...
			if(this.props.widget){
				var component = window[this.props.widget.type];
				//WidgetManager.addWidget(this.props.widget.type, component);
				WidgetList[this.props.widget.type] = component;
				this.DetailWidget = WidgetList[this.props.widget.type];
			}else{
	    ...
	    return { data: this.props.widget.data, showModal: false, loading: true };
  },
  ...
  componentWillReceiveProps: function(nextProps) {
    ...
    if(nextProps.widget.type == this.props.widget.type && isEqual(nextProps.widget.params, this.props.widget.params)){
      this.setState({showModal: false, loading: this.props.loading});
      return;
    }
    ...
    this.setState({ data: nextProps.widget.data, showModal: false, loading: nextProps.loading });
  ...
  refreshWidget: function(props) {
    ...
    if(props.widget.ajax == "get"){
      ...
      var url = this.DetailWidget.prepareUrl(props.widget.params, props.widgetDataUrl);
      $.get(url, function(result) {
        this.setState({data: result, isRefreshed:true});
      }.bind(this), "json" );
  ...
  render: function() {
    ...
    return (
      <div style={widgetStyle} id='dbWidget'>
          <div className="panel panel-default">
            <div className="panel-heading">
              <p>
                <strong>{this.props.widget.title}</strong>
                <span className="pull-right">
                  <a title="reload widget content" style={aTagStyle} onClick={this.refreshWidget.bind(this, this.props)}> <i className="glyphicon glyphicon-refresh"></i> </a>
                </span>
                <span className="pull-right">
            		<sub style={{'font-size':'100%'}}><i>{this.props.widget.subtitle}</i></sub>&nbsp;&nbsp;&nbsp;
            	</span>
              </p>
            </div>
            <div className="panel-body">
              <div style={panelBodyStyle}>
                <this.DetailWidget data={this.state.data} widget={this.props.widget} onClick={this.props.onClick} loading = {this.state.loading} widgetDataUrl = {this.props.widgetDataUrl}
                height = {this.props.widgetHeight} width={this.props.widgetWidth} isRefreshed={this.state.isRefreshed}></this.DetailWidget>
              </div>
            </div>
          </div>
